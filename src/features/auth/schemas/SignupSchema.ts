...
export const SignupSchema = z
	.object({
		name: z
			.string()
			.trim()
			.min(1, 'Neįvestas vardas')
			.min(2, 'Vardas yra per trumpas (min. 2 simboliai)')
			.max(24, 'Vardas yra per ilgas (max. 24 simboliai)')
			.refine((name) => !/^(admin|be\s?[žz]alos)$/i.test(name), { message: 'Toks vardas negalimas' }),
		email: z
			.string()
			.trim()
			.pipe(
			...
			.refine(
				...
				{ message: 'Patikrink el. pašto adresą - atrodo klaidinga galūnė' },
			),
		password: z
			.string()
			.trim()
		...
		initialTarget: z.enum(['profilis', 'virtuve', 'abu', 'nezinau'], {
			error: 'Pasirink tikslą',
		}),
	})
