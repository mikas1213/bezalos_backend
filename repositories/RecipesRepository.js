	...
	async createRecipe(recipeDTO, products) {
		try {
			...
			const insert_products_query =
				'INSERT INTO recipe_products (recipe_id, product_id, grams, created_at) VALUES ($1, $2, $3, $4)';

			for (const prod of JSON.parse(products)) {
				recipe_date.setSeconds(recipe_date.getSeconds() + 1);
				await this.db.query(insert_products_query, [
					recipe_id,
					prod.product_id,
					prod.grams,
					recipe_date.toLocaleString('lt-LT'),
				]);
			}
