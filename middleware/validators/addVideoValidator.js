...
const allow_fields = ['id', 'video_type', 'category', 'title', 'description', 'search_tag', 'duration', 'is_active', 'video', 'photo', 'image_s3_key', 'video_s3_key', 'action'];
...
exports.addVideoFormValidator = catchAsync(async (req, res, next) => {
    ...
    const queries = {
        ...
        'update': { query: 'SELECT * FROM videos WHERE slug = $1 AND id != $2', values: [slug, req?.params?.id] }
    };
    
    try {
        /* F I EL D   V A L I D A T I O N */
        ...
        if(!['Kursai', 'Vebinaras', 'Trumpai'].includes(req.body.category)) throw new ValidationError('Neteisinga video kategorija! 🎬');
