...
exports.createCheckoutSession = async (req, res) => {
	...
	try {
		const session = await stripeSubscriptionSession(
			user_id,
			req.user_name,
			prices_ids[plan_price],
			plan_name,
		);
...
exports.createServiceSession = async (req, res) => {
	...
	try {
		const session = await stripeServiceSession(
			user_role,
			user_id,
			req.user_name,
			service,
...
exports.paymentSuccess = async (req, res) => {
	...
	if (
		...
		const subs_start = new Date(subscription.current_period_start * 1000).toLocaleString(
			'lt-LT',
		...
		await db.query(
			...
			[
				userId,
				subscription.id,
				data.object.metadata.subscription_status,
				subs_start,
				subs_end,
			],
		);
	...
	if (event_type === 'customer.subscription.updated') {
		...
		const subscription_status = !data.object.cancel_at
			? price.metadata.u_plan
			: `Cancel_${price.metadata.s_plan}`;
		await db.query(
	...
	if (event_type === 'customer.subscription.deleted') {
		...
		const subscription_status =
			plan === 'virtuve' && reason === 'payment_failed' ? 'UNPAID' : `Canceled_${plan}`;
		...
		await db.query('DELETE from subscriptions WHERE stripe_subscription_id = $1', [
			data.object.id,
		]);
	}
	...
	if (
		...
		await db.query('UPDATE services SET quantity = quantity - $1 WHERE id = $2', [
			1,
			paslauga_id,
		]);
		await db.query(
			'INSERT INTO orders(user_id, title, price, promo_code, service_id) VALUES($1, $2, $3, $4, $5)',
			[user_id, title, current_price, code, paslauga_id],
		);

		if (JSON.parse(isCodeApproved)) {
			await db.query(
				'UPDATE promotions SET usage_count = usage_count + 1 WHERE promo_code = $1',
				[code],
			);
		}
